...
// Lazy-load chart (uses recharts which is heavy) and PDF
const ComplianceChart = dynamic(
  () => import("@/components/reports/compliance-chart").then((m) => ({ default: m.ComplianceChart })),
...
export default function ReportPage({ params }: PageProps): React.ReactNode {
  const { id } = use(params);
  return <ReportView assessmentId={id} />;
}

function ReportView({ assessmentId }: { assessmentId: string }): React.ReactNode {
  ...
  // Build assessment and template objects from query data
  const { assessment, template, responses } = useMemo(() => {
    if (!query.data) {
      return { assessment: null, template: null, responses: [] as AssessmentResponse[] };
    }

    const rawResponses = (query.data.responses ?? []).map(
      (r: { framework: string; controlId: string; status: string; notes: string | null; evidence: string | null }) => ({
    ...
    const responseMap = Object.fromEntries(
      rawResponses.map((r: AssessmentResponse) => [`${r.framework}::${r.controlId}`, r]),
    );

    const controls: FrameworkControl[] = (query.data.template?.controls ?? []).map(
      (c: { framework: string; controlId: string; controlName: string; description?: string | null; domain: string }) => ({
        id: c.controlId,
        name: c.controlName,
    ...
    const asmt: Assessment = {
      ...
      status: query.data.status === "COMPLETED" ? "completed" : query.data.status === "ARCHIVED" ? "archived" : "in_progress",
      responses: responseMap,
      createdAt: query.data.createdAt ?? "",
      updatedAt: query.data.updatedAt ?? "",
    };

    return { assessment: asmt, template: tmpl, responses: rawResponses };
  }, [query.data]);
  ...
  const reportData = useMemo(() => {
    ...
    return {
      ...
      domainBreakdowns: getDomainBreakdown(template.controls, assessment.responses),
      findings: buildFindingRows(template.controls, assessment.responses),
      remediation: buildRemediationItems(template.controls, assessment.responses),
    };
  }, [assessment, template]);
  ...
  const handleExportCsv = useCallback(() => {
    ...
    try {
      ...
      downloadFile(csv, `${assessment.name.replace(/\s+/g, "-")}-report.csv`, "text/csv;charset=utf-8");
  ...
  const handleExportJson = useCallback(() => {
    ...
    try {
      ...
      downloadFile(json, `${assessment.name.replace(/\s+/g, "-")}-report.json`, "application/json");
  ...
  const handleExportHtml = useCallback(() => {
    ...
    try {
      ...
      downloadFile(html, `${assessment.name.replace(/\s+/g, "-")}-report.html`, "text/html;charset=utf-8");
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="mt-1 text-sm text-ink-muted">
          {query.error?.message ?? "Could not load assessment data."}
        </p>
